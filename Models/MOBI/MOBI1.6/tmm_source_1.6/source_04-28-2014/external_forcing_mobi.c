...
PetscScalar *localsgmask, *localsgbathy;
...
//  ierr = VecLoadVecIntoArray(TR1,"dz.petsc",localdz);CHKERRQ(ierr);
  ...
  ierr = PetscMalloc(lNumProfiles*sizeof(PetscScalar),&localsgmask);CHKERRQ(ierr);  
  ierr = readProfileSurfaceScalarData("sgmask.bin",localsgmask,1);  
...
#endif      
#  if defined O_npzd_nitrogen
               &localsgbathy[kl],&localsgmask[ip],
#  endif
