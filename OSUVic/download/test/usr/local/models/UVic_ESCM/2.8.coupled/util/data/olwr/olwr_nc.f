...
!=======================================================================
!     creates outgoing longwave files olwr_mth.nc and olwr_ann.nc
!=======================================================================
...
!=======================================================================
        ...
        call openfile (trim(path)//'ulwrftop.mon.ltm.nc', iou)
        call getvara ('lon', iou, id, (/1/), (/id/), xd, 1., 0.)
        call getvara ('lat', iou, jd, (/1/), (/jd/), yd, 1., 0.)
!       flip latitude
        yd(1:jd) = yd(jd:1:-1)
        call getvara ('ulwrf', iou, id*jd, (/1,1,n/), (/id,jd,1/)
     &,   data(:,:), 1., 0.)
...
!=======================================================================
     ...
     &, 'latitude of the t grid', 'latitude', 'degrees_north')
      call defvar ('olwr', iou, 3, (/id_xt,id_yt,id_time/), 0., 400.
     &, ' ', 'F', 'TOA outgoing longwave radiation', ' ' ,'W m-2')
      call enddef (iou)
      call putvara ('xt', iou, imt, (/1/), (/imt/), xt, 1., 0.)
      call putvara ('yt', iou, jmt, (/1/), (/jmt/), yt, 1., 0.)
      do n=1,12
        call putvars ('time', iou, n, real(n), 1., 0.)
        call putvara ('olwr', iou, imt*jmt, (/1,1,n/),(/imt,jmt,1/)
     &, var(:,:,n), 1., 0.)
      enddo
      call closefile (iou)
...
!=======================================================================
     ...
     &, 'longitude of the t grid', 'longitude', 'degrees_east')
      call defvar ('yt', iou, 1, (/id_yt/), 0., 0., 'Y', 'F'
     &, 'latitude of the t grid', 'latitude', 'degrees_north')
      call defvar ('olwr', iou, 2, (/id_xt,id_yt/), 0., 400.
     &, ' ', 'F', 'TOA outgoing longwave radiation', ' ' ,'W m-2')
      call enddef (iou)
      call putvara ('xt', iou, imt, (/1/), (/imt/), xt, 1., 0.)
      call putvara ('yt', iou, jmt, (/1/), (/jmt/), yt, 1., 0.)
      call putvara ('olwr', iou, imt*jmt, (/1,1/),(/imt,jmt/)
     &, avar(:,:), 1., 0.)
      call closefile (iou)
