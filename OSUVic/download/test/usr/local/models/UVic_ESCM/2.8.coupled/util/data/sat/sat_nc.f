...
!=======================================================================
!     creates surface air temperature files sat_mth.nc and sat_ann.nc
!=======================================================================
...
!       flip latitude
        yd(1:jd) = yd(jd:1:-1)
        call getvara ('air', iou, id*jd, (/1,1,n/), (/id,jd,1/)
     &,   data(:,:), 1., 273.15)
...
!=======================================================================
     ...
     &, 'latitude of the t grid', 'latitude', 'degrees_north')
      call defvar ('sat', iou, 3, (/id_xt,id_yt,id_time/), -100., 400.
     &, ' ', 'F', 'surface air temperature', ' ' ,'K')
      call enddef (iou)
      call putvara ('xt', iou, imt, (/1/), (/imt/), xt, 1., 0.)
      call putvara ('yt', iou, jmt, (/1/), (/jmt/), yt, 1., 0.)
      do n=1,12
        call putvars ('time', iou, n, real(n), 1., 0.)
        call putvara ('sat', iou, imt*jmt, (/1,1,n/),(/imt,jmt,1/)
     &, var(:,:,n), 1., 0.)
      enddo
      call closefile (iou)
...
!=======================================================================
     ...
     &, 'longitude of the t grid', 'longitude', 'degrees_east')
      call defvar ('yt', iou, 1, (/id_yt/), 0., 0., 'Y', 'F'
     &, 'latitude of the t grid', 'latitude', 'degrees_north')
      call defvar ('sat', iou, 2, (/id_xt,id_yt/), -100., 400.
     &, ' ', 'F', 'surface air temperature', ' ' ,'K')
      call enddef (iou)
      call putvara ('xt', iou, imt, (/1/), (/imt/), xt, 1., 0.)
      call putvara ('yt', iou, jmt, (/1/), (/jmt/), yt, 1., 0.)
      call putvara ('sat', iou, imt*jmt, (/1,1/),(/imt,jmt/)
     &, avar(:,:), 1., 0.)
      call closefile (iou)
